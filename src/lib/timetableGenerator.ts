import { Course, Faculty, Room, TimeSlot, ExamSlot, TimetableConstraints } from './types';
...
export class TimetableGenerator {
  ...
  private addHours(timeStr: string, hours: number): string {
    ...
    const totalMinutes = (hour + Math.floor(hours)) * 60 + minute + Math.round((hours % 1) * 60);
  ...
  private isSlotAvailable(slot: TimeSlot, courseId: string): boolean {
    try {
      ...
      const slotEndMinutes = this.timeToMinutes(this.addHours(slot.startTime, course.duration));
      const slotEndWithGapMinutes = slotEndMinutes + this.constraints.examTimeGap * 60;
      ...
      const conflictingSlots = this.generatedSlots.filter(existingSlot => {
        try {
          ...
          const existingCourse = this.courses.find(c => c.id === existingSlot.courseId);
          if (!existingCourse) {
            console.warn(`Existing course not found: ${existingSlot.courseId}`);
            return false;
          }

          const existingStartMinutes = this.timeToMinutes(existingSlot.startTime);
          const existingEndMinutes = this.timeToMinutes(this.addHours(existingSlot.startTime, existingCourse.duration));
          const existingEndWithGapMinutes = existingEndMinutes + this.constraints.examTimeGap * 60;

          return (
            (slotStartMinutes >= existingStartMinutes && slotStartMinutes < existingEndWithGapMinutes) ||
            (slotEndWithGapMinutes > existingStartMinutes && slotEndWithGapMinutes <= existingEndWithGapMinutes) ||
            (slotStartMinutes <= existingStartMinutes && slotEndWithGapMinutes >= existingEndWithGapMinutes)
          );
        } catch (error) {
  ...
  private getAvailableInvigilators(slot: TimeSlot, course: Course): string[] {
    try {
      const availableFaculty = this.faculty
        .filter(f => {
          try {
            // Check faculty availability
            const isAvailable = f.availableSlots.some(availableSlot =>
              this.isSameDay(availableSlot.date, slot.date) &&
              this.isTimeInRange(slot.startTime, availableSlot.startTime, availableSlot.endTime) &&
              this.isTimeInRange(this.addHours(slot.startTime, course.duration),
            ...
            const dailyInvigilationHours = this.generatedSlots
              ...
              .reduce((total, examSlot) => {
                const examCourse = this.courses.find(c => c.id === examSlot.courseId);
                return total + (examCourse?.duration || 0);
              }, 0);

            return isAvailable && (dailyInvigilationHours < f.maxInvigilationHoursPerDay);
          } catch (error) {
  ...
  private getAvailableRoom(slot: TimeSlot, requiredCapacity: number): Room | null {
    try {
      const availableRooms = this.rooms.filter(room =>
        ...
        !this.generatedSlots.some(examSlot =>
          this.isSameDay(examSlot.date, slot.date) &&
          examSlot.room === room.id &&
          (
            this.isTimeInRange(slot.startTime, examSlot.startTime, examSlot.endTime) ||
            this.isTimeInRange(this.addHours(slot.startTime, this.courses.find(c => c.id === examSlot.courseId)?.duration || 0),
  ...
  public generateTimetable(): ExamSlot[] {
    try {
      ...
      const sortedCourses = [...this.courses].sort((a, b) => {
        ...
        const studentDiff = b.registeredStudents.length - a.registeredStudents.length;
      ...
      for (const course of sortedCourses) {
        ...
        while (!slotAssigned && attempts < maxAttempts) {
          ...
          for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
            ...
            // Skip excluded dates
            if (this.constraints.excludeDates.some(d => this.isSameDay(d, date))) {
              continue;
            }

            const { hour: startHour } = this.parseTime(this.constraints.workingHours.start);
            const { hour: endHour } = this.parseTime(this.constraints.workingHours.end);

            // Try each hour in the working day
            for (let hour = startHour; hour <= endHour - course.duration; hour++) {
              const slot: TimeSlot = {
                date: new Date(date),
              ...
              if (this.isSlotAvailable(slot, course.id)) {
                const invigilators = this.getAvailableInvigilators(slot, course);
                const room = this.getAvailableRoom(slot, course.registeredStudents.length);
        ...
        if (!slotAssigned) {
          console.warn(`Could not assign slot for course: ${course.code} after ${maxAttempts} attempts`);
        }
      }
